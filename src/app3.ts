interface Product {
	...
	sell_status: "available" | "unavailable"; // якщо можливі лише ці значення
	status: "active" | "inactive"; // якщо можливі лише ці значення
	usd_price: string;
	...
	max_price: number;
	has_shops: boolean;
	info: any | null; // якщо можливо будь-яке значення або null
	show_in_site: any | null; // якщо можливо будь-яке значення або null
}
...
// 4) сформувати новий список об”єктів тільки доступних  для продажу товарів, які міститимуть тільки ідентифікатор товару (id), нову ціну (price),
// стару ціну (old_price), та ціну у доларах (usd_price)

// 1) Загальну вартість (нові ціни - price)
function getSumPrice(data: Product[] = dataList): number {
...
function getresizePrice(data: Product[] = dataList): number {
	return data.reduce(
		(acc, product) => (product.price < product.old_price ? acc + 1 : acc),
		0,
	);
}
const resizePrice = getresizePrice();
console.log(resizePrice);

// 3) Товари, які доступні (sell_status:"available")
function getAvailableProducts(data: Product[] = dataList): Product[] {
	return data.filter((product) => product.sell_status === "available");
}
...
function getNewAvailableProducts(data: Product[] = dataList): ProductNew[] {
...
const newProducts: ProductNew[] = getNewAvailableProducts();
