...
// 1) Загальну вартість (нові ціни - price)
function getSumPrice(data = dataList) {
    return data.reduce((acc, product) => acc + product.price, 0);
}
const sumPrice = getSumPrice();
console.log(sumPrice);
// 2)Знайти кількість товарів, у яких ціна зменшилась (price < old_price).
function getresizePrice(data = dataList) {
    return data.reduce((acc, product) => (product.price < product.old_price ? acc + 1 : acc), 0);
}
const resizePrice = getresizePrice();
console.log(resizePrice);
// 3) Товари, які доступні (sell_status:"available")
function getAvailableProducts(data = dataList) {
...
//  4) сформувати новий список об”єктів тільки доступних  для продажу товарів, які міститимуть тільки ідентифікатор товару (id), нову ціну (price),
// стару ціну (old_price), та ціну у доларах (usd_price)
function ProductConstructor(id, old_price, old_usd_price, price) {
    return {
        id,
        old_price,
        old_usd_price,
...
function getNewAvailableProducts(data = dataList) {
    const products = [];
    data.forEach((product) => {
        if (product.sell_status === "available") {
            const newProduct = ProductConstructor(product.id, product.old_price, product.old_usd_price, product.price);
...
const newProducts = getNewAvailableProducts();
